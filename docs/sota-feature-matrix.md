...
| **expert cache policy** | ⭐ **THE DEFICIT IS ONE CONSTANT, NOT THE AXIS — found by adversarially refuting my own first explanation.** Replaying a 9,888-access Qwen3.6-35B-A3B routing trace through the SHIPPED `ExpertCache`, a reference LRU and Belady: at/above the working-set floor hotness-LFU trails LRU by 1.7/7.1/5.0/0.8 points at 96/192/384/768, and below it scores 0.7–12.7% where LRU is arithmetically **0.0%**. ⛔ My published mechanism ("a cell is touched ~3 times so frequency is noise") was WRONG: the policy never sees a whole-trace count — `DECAY_TOKENS=16` halves hotness every 16 tokens while a cell is selected `k/n_expert = 0.5` times per window, so the ranking key holds 0 or 1 almost always. ⭐ Parameterising the decay rate settles it: **d=4 BEATS LRU (41.1 vs 40.4)**, d=0 (true LFU, never decay) is WORST at 25.9%, and d=16 converges to LRU to a tenth of a point — the sanity check the sweep needed. So frequency is the right key with the wrong horizon, and one fixed `DECAY_TOKENS` cannot serve both sides of the floor. ⚠ Floor is 96 only because `FERRIC_MAX_LAYERS=12` truncated the model; deployed is ~320 — read the sweep by RATIO. ⚠ Unseparated: six prompts concatenated into one never-reset cache |
| **expert speculation** | ⛔ **MEASURED AND REFUTED, so not built.** Colibri justifies speculative expert prefetch with "routing is 71.6% predictable one layer ahead". On Qwen3.6-35B-A3B (12 MoE layers, 506 held-out pairs): identity L→L+1 scores **2.7% against a 3.1% random floor** — at chance, uniformly across every layer (0–5%), and beaten nearly 4x by *static per-layer popularity* which requires no prediction at all. ⭐ **The structure is TEMPORAL, not cross-layer**: the same layer's choice for the PREVIOUS TOKEN predicts **41.0%**, 15.2x identity and 13.1x the floor. Routing follows the hidden state, which changes slowly across tokens and completely across layers. ⭐ That is exactly the axis `ExpertCache`'s hotness-LFU already bets on — the measurement CONFIRMS the shipped cache and REFUTES the addition. ⚠ One checkpoint; Colibri measured GLM-5.2, so this shows the premise does not transfer, not that their number is wrong for their model. `route_trace` exists so anyone can run it on their own weights |
| **weighted mirror across devices** | ⭐ **`ferric_tier::MirroredBacking`** — Colibri's "weighted mirror/split routing", where every device holds a FULL copy and each read is divided in proportion to measured bandwidth. No device holds a fraction, so losing one costs speed and not correctness. ⛔ **It also closes a silent-corruption mode Colibri does not mention**: if the copies are not identical — a stale mirror, an interrupted copy, two quantisations of one model — a split read returns the right LENGTH with bytes stitched from two versions, and every component `Backing` honoured its contract while the composite broke it. `verify()` reads probes from EVERY device and compares; `spread_probes` puts the last probe flush with EOF, because a truncated copy diverges late and is invisible to probes near zero; verifying with no probes is REFUSED rather than passing. ⚠ Split by BYTES, not by `split_n` — inside one read the unit is divisible, so the continuous answer is simply correct; the integer solver is for indivisible experts. ⚠ Reads must actually OVERLAP or splitting is strictly worse than not (scoped threads; not wasm) |
| **heterogeneous fabric split** | ⭐ **`ferric_tier::fabric`** — when `m` experts are missing from device memory, some cross the bus and the rest compute IN PLACE on the host, concurrently. Latency-optimal is FreeToken's `q* = m·BP/BH`, derived from the residual host bandwidth `BR = BH − BP`. ⭐ **Energy gives a different answer BY KIND**: `E(q) = q·S·e_dev + (m−q)·S·e_host` is LINEAR in q, so its optimum is always a CORNER (all-bus or all-host) while the latency optimum is interior — they cannot coincide unless the paths cost the same per byte. The useful point is neither: minimise joules subject to a latency budget, verified against brute force at 120 budgets × 3 energy models. ⛔ `split_for_latency` shipped ROUNDING TO NEAREST and was 5% slower than its own claimed minimum — the latency V is asymmetric (falls at S/BR, rises at S/BP), so when BR < BP the optimum is the CEILING, not the nearest. Caught by the profiler printing a faster plan two lines under the one labelled "min latency". ⚠ `FabricProfile` has NO `Default`: measured or nothing. ⭐ **Generalised to N paths** (`split_n`), which is what Colibri's dual-SSD striping needs and what any fabric with more than two ways to move a byte needs: assign items one at a time to whichever path finishes soonest — exactly optimal for unit jobs on uniform machines, and checked against EXHAUSTIVE enumeration rather than asserted. ⭐ The closed form and the greedy are two independently written algorithms for one quantity and must agree on two paths; if they ever diverge one is wrong, which is worth more than either checking itself. ⚠ Colibri's "9 GB/s + 3 GB/s reads ~33% faster than the fast drive alone" is 12/9 = EXACTLY perfect linear aggregation — the ceiling, not a law: two drives behind one root complex add to less than their sum, and `additive_ceiling` exists to be compared against a measured aggregate, never used as one |
