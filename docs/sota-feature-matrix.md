...
| **expert cache policy** | ⚠ **MEASURED ON REAL WEIGHTS, AND IT CUTS BOTH WAYS.** Replaying a 9,888-access Qwen3.6-35B-A3B routing trace through the SHIPPED `ExpertCache`, a reference LRU and Belady's optimum: below the working-set floor (96 = n_layers x top_k) LRU is **0.0%** — the cyclic collapse `ferric-tier` predicts — while hotness-LFU scores 6.3–12.7%, so frequency-primary is exactly what rescues that regime. ⛔ **Above the floor it LOSES to plain LRU at every capacity**, by up to 7.2 points (33.2% vs 40.4% at 192); "gap closed" is negative. Mechanism follows from the routing measurement: a (layer, expert) is touched ~3 times in the whole trace, so frequency is mostly noise while recency carries 41% — the cache is keyed on the weaker axis. ⚠ One checkpoint, prefill-derived routing replayed token-major; the crate's cited "LRU 36.24%, dead flat" is a larger model whose working set exceeded every budget tested, i.e. entirely the regime where hotness-LFU wins here. Both can be true — what is missing is anything deciding WHICH SIDE of the floor a deployment is on |
