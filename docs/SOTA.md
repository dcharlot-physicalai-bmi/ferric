...
**Whole-block fusion IS a win — fused FFN gate/up + SwiGLU (2026-07-19).** The prediction above ("only
megafusion pays") tested and confirmed. `matmul_q4_k_swiglu` computes the gate row `o` and up row
`o+n_ff` in one kernel and writes `silu(gate)·up` directly — no `[t, 2·n_ff]` intermediate, no separate
SwiGLU dispatch, and the projection writes `n_ff` outputs instead of `2·n_ff`. Wired into the FFN for
single-shard gate_up across **the whole k-quant family** — `matmul_q{4,5,6}_k_swiglu`, dispatched by
`try_matmul_swiglu` (Q4_K_M / Q5_K_M / Q6_K all covered; mixed/other shards fall back). Three lessons, all measured:
1. **It's exact and correct** — the fused kernel is token-for-token identical to llama.cpp on all three real models (Qwen3-0.6B Q4_K_M, Qwen3-0.6B Q5_K_M, Qwen2.5-0.5B Q6_K), and `examples/ffn_fuse.rs` validates each kernel vs `matmul_qk(w).swiglu()` at max|Δ|/scale ≤ 4e-6.
2. **Microbench lied, end-to-end told the truth.** The isolated kernel bench swung from −9% to +6× depending on shape — that weight is ~3.5 MB, cache-resident, exactly the trap this doc warns about. Ignore it.
3. **The whole-block win is real and transfers across formats.** Controlled interleaved A/B *in one binary* (`FERRIC_NOFUSE` env forces the un-fused path, alternated rep-by-rep to kill thermal drift): Q4_K_M **25→24**, Q5_K_M **27→26**, Q6_K **~21→~19/~19→18/~22→20 ms/tok** — a consistent **~4–8% decode win** on every model, 3/3 reps each. Unlike `add_rmsnorm`'s within-noise result, it transfers because fusing a whole block removes real memory traffic (the `2·n_ff` intermediate) not just a dispatch.
So: single-op fusion is noise; **whole-block fusion is a measured win, now across the entire k-quant FFN family**.

**But the FULL FFN megakernel is a measured LOSS — the occupancy wall (2026-07-19).** The natural next step was folding the down projection in too: one kernel does gate_up → SwiGLU → down, keeping the `n_ff`-wide activation in **workgroup shared memory** so the intermediate never touches global and a whole dispatch disappears. Built it for the Q4_K_M pair (Q4_K gate_up + Q6_K down, `ffn_mega_q4k_q6k` / `try_ffn_mega`). It is **token-for-token identical to llama.cpp** — and **~2× slower**: 46–47 vs 22–28 ms/tok (Qwen3-0.6B-Q4_K_M, interleaved, 4/4 reps). The reason is structural: keeping `hff` in shared memory forces **one workgroup per token**, so at batch-1 decode a single core does the entire FFN while the rest of the GPU sits idle. That occupancy loss dwarfs the dispatch + intermediate-traffic it saves. This refutes the tempting "fewer dispatches always win at decode" intuition head-on — dispatch count is *a* cost, but under-occupancy is a bigger one. The natural rescue — "at *prefill* the one-workgroup-per-token design fills the GPU, so it should win there" — was **also tested and disproven**: on a ~400-token prompt, prefill is a wash (mega ≈ staged, within heavy run-to-run noise), because the staged path's prefill matmuls already use the cooperative-matrix (tensor-core) kernels the megakernel's scalar down-dot can't match. So the megakernel wins *nowhere*. The kernel is kept **opt-in behind `FERRIC_MEGA`** (off by default, the staged fused-SwiGLU + down path stays default) as a documented negative; making it pay would need a batched / multi-workgroup redesign (e.g. split-K down with a global `hff`, which reintroduces the very traffic the fusion removed). The honest boundary: **fuse a block's element-wise tail into its matmul (a win); do not collapse two independent matmuls whose parallelism you'd have to throw away (a loss).**
