//! π0 FULL flow-matching action ROLLOUT in pure Rust: chain the verified Gemma-300m expert through the
//! rectified-flow Euler denoise loop (10 steps, x_t += dt·v, t: 1→0) to generate a complete action chunk,
//! verified vs the real lerobot PI0 `sample_actions`. usage: cargo run -p ferric-llama --example pi0_rollout --release
...
async fn run() {
    ...
    let jg = json_min::parse(&std::fs::read_to_string(format!("{home}/.cache/ferric/cosmos_ref/pi0_rollout_golden.json")).unwrap());
    ...
    let num_steps = jg.get("num_steps").as_usize();
    ...
    let state_emb = lin(&state, "state_proj", pd);

    // preload prefix KV (fixed across steps) + block mask
    let mut pks = Vec::new(); let mut pvs = Vec::new();
    for l in 0..18 {
        let kv = jg.get("kv").idx(l);
        pks.push(Tensor::from_vec(&ctx, &kv.get("k").as_f64_vec().iter().map(|x| *x as f32).collect::<Vec<_>>(), &[plen, hd]));
        pvs.push(Tensor::from_vec(&ctx, &kv.get("v").as_f64_vec().iter().map(|x| *x as f32).collect::<Vec<_>>(), &[plen, hd]));
    }
    let tk = plen + n;
    let mut mask = vec![0f32; n * tk];
    for qi in 0..n { for kj in 0..n { let allowed = if qi == 0 { kj == 0 } else { true }; if !allowed { mask[qi * tk + (plen + kj)] = -1e30; } } }
    let mask_t = Tensor::from_vec(&ctx, &mask, &[n, tk]);

    // expert forward: (x_t [chunk,adim], timestep) -> velocity [chunk,adim]
    let half = pd / 2;
    let forward = |x_t: &[f64], ts: f64| -> Vec<f32> {
        let xt = Tensor::from_vec(&ctx, &x_t.iter().map(|v| *v as f32).collect::<Vec<_>>(), &[chunk, adim]);
        let mut te = vec![0f32; pd];
        for i in 0..half { let frac = i as f64 / (half - 1) as f64; let period = minp * (maxp / minp).powf(frac);
            let sc = 1.0 / period * 2.0 * std::f64::consts::PI; te[i] = (sc * ts).sin() as f32; te[half + i] = (sc * ts).cos() as f32; }
        let ae = pollster::block_on(lin(&xt, "action_in_proj", pd).to_vec());
        ...
        let action_time = lin(&lin(&action_time, "action_time_mlp_in", pd).silu(), "action_time_mlp_out", pd);
        let mut x = state_emb.cat(&action_time, 0);
        for l in 0..18 {
            ...
            let o = nn::masked_attention_kv(&q, &pks[l].cat(&k, 0), &pvs[l].cat(&v, 0), &mask_t, nh, nkv).matmul_bt(&b("self_attn.o_proj.weight"));
            x = x.add(&o);
        ...
        pollster::block_on(lin(&x.narrow(0, 1, chunk), "action_out_proj", adim).to_vec())
    };

    // rectified-flow Euler: dt = -1/steps; x_t = noise; x_t += dt·v; t: 1 -> 0
    let dt = -1.0 / num_steps as f64;
    let mut x_t: Vec<f64> = jg.get("noise").as_f64_vec();
    for step in 0..num_steps {
        let time = 1.0 + step as f64 * dt;
        let v = forward(&x_t, time);
        for i in 0..x_t.len() { x_t[i] += dt * v[i] as f64; }
    }
    let fr = jg.get("final").as_f64_vec();
    let mut e = 0.0f64; let mut se = 0.0f64;
    for i in 0..chunk * adim { let d = (x_t[i] - fr[i]).abs(); e = e.max(d); se += d; }
    let mean = se / (chunk * adim) as f64;
    println!("π0 full flow rollout ({num_steps} steps) vs real sample_actions: maxΔ={e:.3e}  meanΔ={mean:.3e}  ->  {}", if mean < 3e-3 { "MATCH ✓" } else { "MISMATCH ✗" });
    assert!(mean < 3e-3, "π0 rollout diverged");
}
