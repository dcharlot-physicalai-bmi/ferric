//! PRODUCTION-mode Cosmos 3 Edge action rollout in pure Rust: CFG (cond+uncond, guidance=3) + a real
//! embodiment width (raw_action_dim=10 -> channels 10..64 zeroed each step), through Ferric's packing
//! forward + UniPC for 4 steps. Matches the REAL transformer + REAL UniPC production rollout
//! (`cosmos_cfg_rollout_ref.py`). This closes the last gap — CFG + channel-zeroing verified against the
//! ACTUAL model in the loop (not just synthetically). usage:
//!   cargo run -p ferric-llama --example cosmos_action_cfg_rollout --release -- <cosmos3-edge-dir>
...
async fn run() {
    ...
    let gp = format!("{}/.cache/ferric/cosmos_ref/action_cfg_rollout_golden.json", std::env::var("HOME").unwrap());
    let jg = json_min::parse(&std::fs::read_to_string(&gp).expect("run cosmos_cfg_rollout_ref.py first"));
    let (alen, adim, lc, raw) = (jg.get("ALEN").as_usize(), jg.get("ADIM").as_usize(), jg.get("LC").as_usize(), jg.get("RAW").as_usize());
    let cfg_scale = jg.get("CFG").as_f64();
    let und_len = jg.get("und_len").as_usize();
    let seq_len = jg.get("seq_len").as_usize();
    let cond_ids: Vec<u32> = jg.get("cond_ids").as_f64_vec().iter().map(|x| *x as u32).collect();
    let uncond_ids: Vec<u32> = jg.get("uncond_ids").as_f64_vec().iter().map(|x| *x as u32).collect();
    ...
    let timesteps = jg.get("timesteps").as_f64_vec();
    let final_ref = jg.get("final_action").as_f64_vec(); // [alen*raw]
    ...
    let raww = |n: &str| -> &[f32] { &w[n].data };

    // fixed pieces
    let (pt, ph, pw): (Vec<i64>, Vec<i64>, Vec<i64>) = (
        pos[0].iter().map(|x| *x as i64).collect(), pos[1].iter().map(|x| *x as i64).collect(), pos[2].iter().map(|x| *x as i64).collect());
    ...
    let (cos_und, sin_und) = (cos_t.narrow(0, 0, und_len), sin_t.narrow(0, 0, und_len));
    let (cos_gen, sin_gen) = (cos_t.narrow(0, und_len, seq_len - und_len), sin_t.narrow(0, und_len, seq_len - und_len));
    let embed = g("embed_tokens.weight");
    let und_cond = embed.gather_rows(&cond_ids);     // precompute both prompts' text embeds
    let und_uncond = embed.gather_rows(&uncond_ids);
    let patch_dim = 4 * lc;
    ...
    let fc_in = raww("action_proj_in.fc.weight"); let b_in = raww("action_proj_in.bias.weight");
    let fc_out = raww("action_proj_out.fc.weight"); let b_out = raww("action_proj_out.bias.weight");

    let forward = |und0: &Tensor, lat: &[f64], timestep: f64| -> Vec<f32> {
        ...
        let act_hidden = pollster::block_on(gen_out.narrow(0, 1, alen).to_vec());
    ...
    for i in 0..4 {
        let cond = forward(&und_cond, &lat, timesteps[i]);
        let uncond = forward(&und_uncond, &lat, timesteps[i]);
        // CFG + zero padding channels of the velocity
        let mut v = vec![0f64; alen * adim];
        for a in 0..alen { for c in 0..adim {
            let k = a * adim + c;
            v[k] = if c >= raw { 0.0 } else { uncond[k] as f64 + cfg_scale * (cond[k] as f64 - uncond[k] as f64) };
        }}
        lat = sched.step(&v, &lat);
        for a in 0..alen { for c in raw..adim { lat[a * adim + c] = 0.0; } } // zero padding channels of latents
        println!("  step {i} t={:.0}: latent[0,:5]={:?}", timesteps[i], round(&lat[..5], 5));
    }
    println!("CFG rollout (8× packing forward + UniPC) {:?}", t0.elapsed());
    // slice to raw_action_dim
    let mut out = Vec::with_capacity(alen * raw);
    for a in 0..alen { for c in 0..raw { out.push(lat[a * adim + c]); } }
    let maxerr = out.iter().zip(&final_ref).map(|(x, y)| (x - y).abs()).fold(0.0, f64::max);
    println!("Ferric final (sliced)[0] = {:?}", round(&out[..raw], 5));
    println!("real   final (sliced)[0] = {:?}", round(&final_ref[..raw], 5));
    println!("Ferric sum={:.5}  real sum={:.5}", out.iter().sum::<f64>(), final_ref.iter().sum::<f64>());
    println!("\nMAX Δ vs real CFG+embodiment rollout = {maxerr:.3e}  ->  {}",
             if maxerr < 3e-3 { "MATCH ✓" } else { "MISMATCH ✗" });
    assert!(maxerr < 3e-3, "Ferric CFG rollout diverged from the real Cosmos 3 Edge production rollout");
}
