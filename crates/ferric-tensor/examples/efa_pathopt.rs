//! EFA — close the PATH-EFFICIENCY gap: reward reaching the goal EARLY, not just eventually.
//!
//! The benchmark showed EFA-MPPI reaches 87% of optimal but MEANDERS (~2.5× the shortest path): it scores
//! each rollout by the TERMINAL value V(x_H,g), which rewards *ending up* near the goal after H steps, not
//! *getting there fast*. Principled fix (a per-step reward / TD-MPC2-style discounted return): score a
//! rollout by max_h [ V(x_h,g) − c·(h−1) ] over the horizon — prefer plans that hit high value in the
//! FEWEST steps. We compare terminal-value scoring vs early-reward scoring (sweep the step cost c) against
//! the exact BFS optimal, on reach AND path-length ratio. Same world model + value net (our 87%/2.48× run).
//!
//! Run: `cargo run -p ferric-tensor --example efa_pathopt --release`
...
fn bfs(goal: (usize, usize)) -> Vec<i32> {
    let mut dist = vec![-1i32; IN]; let mut q = VecDeque::new();
    dist[goal.1 * G + goal.0] = 0; q.push_back(goal);
    while let Some(c) = q.pop_front() { let d0 = dist[c.1 * G + c.0]; for a in 0..NA { let nc = next_pos(c, a); let idx = nc.1 * G + nc.0; if dist[idx] < 0 { dist[idx] = d0 + 1; q.push_back(nc); } } }
    dist
}
...
async fn run() {
    ...
    println!("  EFA maze — PATH-EFFICIENCY: terminal-value vs early-reward MPPI scoring. D={D}");
    ...
    let opt_len: Vec<i32> = (0..pos0.len()).map(|i| dists[i][pos0[i].1 * G + pos0[i].0]).collect();

    println!("\n  {:<28}  {:>6}  {:>14}", "scoring (MPPI K=512, H=6)", "reach", "path vs optimal");
    // c < 0 → terminal-value only (the current 2.48× baseline)
    let (r0, p0) = mppi(&ctx, 512, 6, 0.5, -1.0, &pos0, &goal, &opt_len, &predict_t, &vt, &enc_batch).await;
    println!("  {:<28}  {:>5.0}%  {:>13.2}×", "terminal value (baseline)", r0 * 100.0, p0);
    for &c in &[0.0f32, 0.03, 0.06, 0.10] {
        let (r, pr) = mppi(&ctx, 512, 6, 0.5, c, &pos0, &goal, &opt_len, &predict_t, &vt, &enc_batch).await;
        println!("  {:<28}  {:>5.0}%  {:>13.2}×", format!("early-reward  c={c}"), r * 100.0, pr);
    }
    println!("\n  optimal path = 1.00×. early-reward scoring should pull the ratio DOWN (shorter routes) — the honest test");
    println!("  is whether reach holds while paths shorten (c too high → over-greedy → reach drops).");
}
...
// MPPI; cost<0 → score by terminal value; cost>=0 → score by max_h [ V(x_h) − cost·(h−1) ] (early-reward).
async fn mppi(ctx: &Arc<ferric_core::Context>, kk: usize, hh: usize, lambda: f32, cost: f32, pos0: &[(usize, usize)], goal: &[(usize, usize)], opt_len: &[i32], predict_t: &dyn Fn(&Tensor, &Tensor) -> Tensor, val_t: &dyn Fn(&Tensor, &Tensor) -> Tensor, enc_batch: &dyn Fn(&[(usize, usize)]) -> Tensor) -> (f32, f32) {
    ...
    for stp in 0..maxsteps {
        ...
        // roll H steps; collect the score (terminal or early-reward) per candidate
        let mut xh = enc_batch(&pos_rep);
        let mut score = vec![f32::MIN; m];
        for h in 0..hh {
            let mut oh = vec![0.0f32; m * NA]; for r in 0..m { oh[r * NA + acts[h][r]] = 1.0; }
            xh = predict_t(&xh, &Tensor::from_vec(ctx, &oh, &[m, NA]));
            if cost < 0.0 { if h == hh - 1 { let v = val_t(&xh, &xg).to_vec().await; for r in 0..m { score[r] = v[r]; } } }
            else { let v = val_t(&xh, &xg).to_vec().await; for r in 0..m { let s = v[r] - cost * h as f32; if s > score[r] { score[r] = s; } } }
        }
    ...
    (reached.iter().filter(|&&r| r).count() as f32 / t as f32, if rcnt > 0 { rsum / rcnt as f32 } else { 0.0 })
}
