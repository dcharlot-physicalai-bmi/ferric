...
fn mpc_episode(seed: u32, im: f32) -> f32 {
    ...
    best }
// ─────────────────────────────────────────────────────────────────────────────
// DIFFUSION policy (chunked, obs-conditioned) — the literature's actual PushT recipe, and the energy-first approach
// done RIGHT: a denoising score model is a learned energy GRADIENT whose stochastic sampling COMMITS to one mode
// (unlike my energy-argmin, which failed). Denoise a 6-macro-action chunk from noise, conditioned on obs. DDPM.
const HD: usize = 256; const ODP: usize = 12; const CM: usize = 6; const AD: usize = CM * 2; const T: usize = 16;
struct DNet { w: Vec<Vec<f32>>, b1: Vec<f32>, w2: Vec<f32>, b2: Vec<f32>, w3: Vec<f32>, b3: Vec<f32> }   // in = AD(chunk)+1(t)+ODP(obs)
...
// DDPM schedule (linear betas)
fn schedule() -> (Vec<f32>, Vec<f32>, Vec<f32>) { let mut betas = vec![]; for i in 0..T { betas.push(1e-4 + (0.02 - 1e-4) * i as f32 / (T - 1) as f32); }
    let alphas: Vec<f32> = betas.iter().map(|b| 1.0 - b).collect(); let mut abar = vec![]; let mut c = 1.0; for a in &alphas { c *= a; abar.push(c); } (betas, alphas, abar) }
...
async fn run() {
    let im = moment(); let (betas, alphas, abar) = schedule();
    println!("  EFA-2 · PUSHT — DIFFUSION policy (chunk={} macros, {} denoise steps): score model = energy done right\n", CM, T);
    let n_demo = 70u32; let mut ob: Vec<[f32; ODP]> = vec![]; let mut ch: Vec<[f32; AD]> = vec![];
    ...
    let ctx = std::sync::Arc::new(Context::new().await.expect("ctx")); let nin = AD + 1 + ODP; let bs = 256; let np = ob.len();
    let mut pp: Vec<Tensor> = (0..nin).map(|c| Tensor::from_vec(&ctx, &randn(HD, 500 + c as u32, 0.35), &[1, HD])).collect();
    ...
    println!("  training the denoiser ε_θ(a_t, t, obs) (predict the noise; DDPM):");
    for it in 0..20000u32 {
        ...
        for i in 0..bs { let idx = (u(it * 131 + i as u32, 5) * np as f32) as usize % np; let o = ob[idx]; let a0 = ch[idx];
            ...
            for c in 0..AD { let e = gnoise(it * 131 + i as u32, c * 3 + 1); let at = sa * a0[c] + sb * e; cols[c][i] = at; tb[i * AD + c] = e; }
        ...
        if it % 5000 == 4999 { println!("     iter {:>5}: denoise MSE {:.4}", it + 1, loss.value().to_vec().await[0]); } }
    ...
    // DDPM ancestral sampling of a chunk given obs
    let sample = |ob: &[f32; ODP], seed: u32| -> [f32; AD] { let mut a = [0.0f32; AD]; for c in 0..AD { a[c] = gnoise(seed, c); }
        for t in (0..T).rev() { let eps = net.eps(&a, t as f32 / T as f32, ob);
            let inv_sa = 1.0 / alphas[t].sqrt(); let coef = (1.0 - alphas[t]) / (1.0 - abar[t]).sqrt();
            for c in 0..AD { a[c] = inv_sa * (a[c] - coef * eps[c]); }
            if t > 0 { let sig = betas[t].sqrt(); for c in 0..AD { a[c] += sig * gnoise(seed.wrapping_add(9973 + t as u32), c); } } }
        a };
    println!("\n  the card — DIFFUSION policy, PushT (200 held-out; sample a chunk, execute open-loop, re-sample):");
    ...
    println!("     [anchors] MPC 0.69 · flow 0.078 · history 0.094 · energy-argmin 0.070 · chunk-regress 0.125 · scripted 0.20 · ES 0.28");
    println!("\n  THE TEST: a diffusion (score) head SAMPLES one mode instead of averaging — if this is what the multimodal");
    println!("  demos needed, coverage jumps toward MPC's 0.69. It is the energy-first method done right. One seed.");
}
