...
fn mpc_episode(seed: u32, im: f32) -> f32 {
    ...
    best }
// ─────────────────────────────────────────────────────────────────────────────
// DISTILL a fast flow policy from MPC demonstrations (MPC = the teleop substitute). Generate (obs, action) pairs by
// running MPC, then conditional-flow-match a reactive net → one forward pass at deploy vs MPC's 67k rollouts/decision.
const HF: usize = 160; const ODP: usize = 12;
...
fn flow_act(net: &FNet, ob: &[f32; ODP], kk: usize) -> [f32; 2] { let mut a = [0.0f32; 2];
...
// run MPC on one episode, recording (obs, executed macro-action) at each physics step
fn mpc_demo(seed: u32, im: f32, obs_buf: &mut Vec<[f32; ODP]>, act_buf: &mut Vec<[f32; 2]>) {
    let mut w = World::new(seed, im); let mut mean = vec![[0.0f32; 2]; HZ];
    let decisions = ((TMAX / DT) as usize) / STRIDE;
    for t in 0..decisions { let a = mpc_act(&w, &mut mean, seed.wrapping_mul(31) + t as u32 * 7919);
        for _ in 0..STRIDE { obs_buf.push(w.obs()); act_buf.push([a[0] / PUSH_V, a[1] / PUSH_V]); w.step(a); }
        if w.coverage() >= 0.98 { break; } }
}
fn main() { pollster::block_on(run()); }
async fn run() {
    let im = moment();
    println!("  EFA-2 · PUSHT — DISTILL a fast flow policy from MPC demonstrations (MPC = teleop substitute)\n");
    // ── generate demos ──
    let n_demo = 50u32; let mut obs_buf: Vec<[f32; ODP]> = vec![]; let mut act_buf: Vec<[f32; 2]> = vec![];
    print!("  generating {} MPC demo episodes… ", n_demo);
    for k in 0..n_demo { mpc_demo(3000 + k, im, &mut obs_buf, &mut act_buf); }
    println!("collected {} (obs,action) pairs", obs_buf.len());
    // ── CFM distillation ──
    let ctx = std::sync::Arc::new(Context::new().await.expect("ctx")); let fin = ODP + 3; let bs = 256; let np = obs_buf.len();
    ...
    println!("  distilling the flow (obs {} → {} → {} → 2; CFM to MPC actions):", ODP, HF, HF);
    for it in 0..16000u32 {
        let mut cols: Vec<Vec<f32>> = (0..fin).map(|_| vec![0.0f32; bs]).collect(); let mut tb = vec![0.0f32; bs * 2];
        for i in 0..bs { let idx = (u(it * 131 + i as u32, 5) * np as f32) as usize % np; let ob = obs_buf[idx]; let un = act_buf[idx];
            ...
            for c in 0..ODP { cols[c][i] = ob[c]; } cols[ODP][i] = (1.0 - t) * a0[0] + t * un[0]; cols[ODP + 1][i] = (1.0 - t) * a0[1] + t * un[1]; cols[ODP + 2][i] = t;
    ...
    // ── eval the fast distilled policy on held-out episodes ──
    println!("\n  the card — DISTILLED flow policy, PushT (coverage; 200 held-out episodes, 1 fwd pass/decision):");
    for kk in [1usize, 2] { let (mut mc, mut ok, mut c5, mut c7) = (0.0f32, 0, 0, 0);
        for k in 500..700u32 { let mut w = World::new(k, im); let mut best = 0.0f32;
            for _ in 0..((TMAX / DT) as usize) { let a = flow_act(&netf, &w.obs(), kk); w.step(a); best = best.max(w.coverage()); if best >= 0.98 { break; } }
    ...
    println!("     [anchors] MPC teacher ~0.69 (17% success, 67k rollouts/decision) · scripted 0.20 · ES 0.28");
    println!("\n  Honest: PushT class + published coverage metric; the distilled flow is ONE forward pass/decision (vs MPC's");
    println!("  ~67k physics-step rollouts) — the teleop(MPC)→fast-policy pipeline. Our T proportions; one seed.");
}
