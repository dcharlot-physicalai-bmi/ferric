//! EFA energy-first #58 — the HYBRID flow: v = −κ∇ₐE + w. Potential carries the field; a small penalized correction
//! closes the fit gap. Best-of-both, measured.
//!
//! The map so far: plain flow actuates 100% (no energy); the pure potential v=−∇ₐE verifies 95% but actuates 25–46%
//! (scalar-fit bottleneck, decisively not numerics). The hybrid keeps the scalar potential Eθ(s,a,t) as the PRIMARY
//! field (and the verify/energy semantics) and adds a SMALL 2-output correction wφ, trained jointly by flow matching
//! with an ℓ2 penalty on w — so the potential must explain as much of the field as it can and w absorbs only the
//! non-conservative/fit residual. We measure (a) actuation reach vs K, (b) verify from Eθ(·,1), and (c) the FIELD SPLIT
//! mean|κ∇ₐE| vs mean|w| — the honest number for "how energy-first is the hybrid." Target: pair-level actuation with a
//! readable energy. HONEST: distilled demonstrator, small body; λ picked once (0.1), not swept.
//!
//! Run: `cargo run -p ferric-tensor --example ebm_hflow --release`
...
// scalar potential (relu/linear) with exact CPU action-gradient
...
// small correction net (relu/linear, 2 outputs)
...
async fn run() {
    ...
    println!("  EFA energy-first — HYBRID flow v = −κ∇ₐE + w: potential carries the field, small penalized correction closes the gap\n");
    ...
    // ---- Stage 2: JOINT training — E (scalar, via ∇ₐ) + w (2-output), v = −κ∇ₐE + w matches (u*−a0); λ‖w‖² keeps w small ----
    ...
    let wnet = |f: &[Var], a1: &Var, a2: &Var, tt: &Var, pv: &[Var]| { let mut pre = pv[11].clone(); for c in 0..8 { pre = pre.add(&f[c].matmul(&pv[c])); }
        pre = pre.add(&a1.matmul(&pv[8])).add(&a2.matmul(&pv[9])).add(&tt.matmul(&pv[10]));
        pre.relu().matmul(&pv[12]).add(&pv[13]).relu().matmul(&pv[14]).add(&pv[15]) };
    ...
    let mut r: Vec<Tensor> = (0..11).map(|c| Tensor::from_vec(&ctx, &randn(H, 120 + c as u32, 0.4), &[1, H])).collect();
    r.push(Tensor::zeros(&ctx, &[H])); r.push(Tensor::from_vec(&ctx, &randn(H * H, 140, 1.0 / (H as f32).sqrt()), &[H, H])); r.push(Tensor::zeros(&ctx, &[H]));
    r.push(Tensor::from_vec(&ctx, &randn(H * 2, 141, 1.0 / (H as f32).sqrt()), &[H, 2])); r.push(Tensor::zeros(&ctx, &[2]));
    ...
    // ---- Stage 3: eval — hybrid actuation, verify from E, and the FIELD SPLIT ----
    let hact = |s: [f32; 4], g: (f32, f32), k: usize| -> (f32, f32) { let (mut a1, mut a2) = (0.0f32, 0.0f32);
        for i in 0..k { let t = i as f32 / k as f32; let (g1, g2) = ef.grad_a(s, g, a1, a2, t); let (w1, w2) = wn.vel(s, g, a1, a2, t);
            a1 += (-KAPPA * g1 + w1) / k as f32; a2 += (-KAPPA * g2 + w2) / k as f32; }
    ...
    let run_disc = || -> f32 { let mut rr = 0; for i in 0..n { let mut s = inits[i]; let g = goals[i]; for _ in 0..260 { let uu = vn.ustar(s, g, &G9); s = step(s, uu.0, uu.1); } if reached(s, g) { rr += 1; } } rr as f32 / n as f32 * 100.0 };
    let run_h = |k: usize| -> f32 { let mut rr = 0; for i in 0..n { let mut s = inits[i]; let g = goals[i]; for _ in 0..260 { let (u1, u2) = hact(s, g, k); s = step(s, u1, u2); } if reached(s, g) { rr += 1; } } rr as f32 / n as f32 * 100.0 };
    // verify from E at t=1
    ...
    // field split at eval states over the K=2 grid
    let (mut me, mut mw) = (0.0f32, 0.0f32); let mut cnt = 0.0f32;
    for i in 0..n { let (mut a1, mut a2) = (0.0f32, 0.0f32); for kk in 0..2 { let t = kk as f32 / 2.0;
    ...
    println!("  v = −κ∇ₐEθ + wφ, trained jointly with λ‖w‖² (λ={}). eval: {} ep × {} goals.\n", LAM, nep, TG.len());
    println!("     reading                                        result");
    println!("     discrete argmin (demonstrator), fine 9×9        {:>4.0}% reach   (81 evals/decision)", run_disc());
    for k in [1usize, 2, 4] { println!("     ACTUATE — hybrid field, K={:<2}                    {:>4.0}% reach", k, run_h(k)); }
    println!("     VERIFY  — Eθ(·,1) ranks good action < bad       {:>4.1}%", vg as f32 / vt as f32 * 100.0);
    println!("     FIELD SPLIT — mean|κ∇ₐE| : mean|w| = {:.2} : {:.2}  → potential carries {:>4.0}% of the field", me / cnt, mw / cnt, me / (me + mw) * 100.0);
    println!("\n  Target: pair-level actuation (~100%) with a readable energy (verify high) and the potential carrying most of the");
    println!("  field — the measured 'how energy-first is the hybrid.' HONEST: λ={} picked once, not swept; distilled demonstrator.", LAM);
}
