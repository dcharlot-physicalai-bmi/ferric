...
//!
//! Run: `cargo run -p ferric-tensor --example ebm_arm3 --release`
...
impl Vn {
    ...
    fn ustar(&self, s: [f32; 6], g: (f32, f32, f32)) -> [f32; 3] { let mut bu = [0.0f32; 3]; let mut be = f32::MAX;
        for &u1 in &G5 { for &u2 in &G5 { for &u3 in &G5 { let uu = [u1, u2, u3]; let ns = step(s, uu); let q = cost(s, g, uu) * DT + GAMMA * self.eval(ns, g); if q < be { be = q; bu = uu; } } } } bu }
}
...
async fn run() {
    ...
    println!("     discrete argmin, 5×5×5             {:>4.0}%         125          Gᵈ — exponential in DOF (9³ would be 729)", rd as f32 / n as f32 * 100.0);
    ...
    println!("\n  At 1-DOF the edge was 5:1, at 2-DOF 25:1 — here matching discrete at K forward passes is worth 125:1 (729:1 vs a");
    println!("  fine grid): the eval-budget compounding of the corrected recipe, demonstrated. HONEST: fully-actuated chain;");
    println!("  underactuated goal-reaching needs a different goal structure (not arbitrary joint targets) — priced, not attempted.");
}
