//! EFA-2 · REACHER (SMOOTH demonstrator) — CONTROLLED TEST of the smoothness law: fix the elbow so the demonstrator
//! is CONTINUOUS (no elbow-switch discontinuity). Prediction: flow K=1 jumps from 63% (discontinuous) toward >=90%,
//! causally proving the K=1 gap was the demonstrator's discontinuity, not the task. Otherwise same as the first task on our VERIFIED Rust articulated-dynamics core (sim_planar).
...
fn pick_ik(x: f32, y: f32, _q: &[f32; 2]) -> [f32; 2] { ik(x, y, 1.0) }      // FIXED elbow → CONTINUOUS demonstrator
                                                                            // (no elbow-switching discontinuity)
...
fn sample_target(seed: u32) -> [f32; 2] {                                // reachable annulus
    let r = 0.4 + u(seed, 1) * 1.3;  // [0.4,1.7], away from the r=2 singularity
    let a = u(seed, 2) * 2.0 * PI; [r * a.sin(), -r * a.cos()] }
