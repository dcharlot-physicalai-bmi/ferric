...
fn main() {
    let im = moment();
    println!("  EFA-2 · PUSHT via CEM-MPC — plan multi-push sequences on the verified physics (the demonstrator)\n");
    let self_cov = { let w = World { p: [0.3, -0.2], th: 0.7, v: [0.0; 2], w: 0.0, tp: [0.3, -0.2], tth: 0.7, im, push: [9.0, 9.0] }; w.coverage() };
    println!("  [V1] coverage self-check {:.3} · moment {:.4} · MPC: H={} samples={} iters={} elite={}", self_cov, im, HZ, NS, NITER, ELITE);
    let (mut mc, mut ok, mut c5, mut c7) = (0.0f32, 0, 0, 0);
    let neps = 30u32;
    for k in 0..neps { let c = mpc_episode(1000 + k, im); mc += c; if c >= COV_OK { ok += 1; } if c >= 0.5 { c5 += 1; } if c >= 0.7 { c7 += 1; } }
    println!("\n  CEM-MPC demonstrator, PushT ({} episodes): mean coverage {:.3} · success(≥0.90) {:.0}% · ≥0.7 {:.0}% · ≥0.5 {:.0}%",
        neps, mc / neps as f32, ok as f32 / neps as f32 * 100.0, c7 as f32 / neps as f32 * 100.0, c5 as f32 / neps as f32 * 100.0);
    println!("     [anchors] scripted planner ~0.20 · ES-from-scratch ~0.28 · MPC above");
    println!("\n  If MPC solves PushT, its trajectories are the demonstration dataset → distill a fast flow policy (next).");
}
